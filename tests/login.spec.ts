...
test('Invalid Login (Incorrect Password)', async ({ page }) => {
  ...
  await login.signIn();
  await login.assertToastMessage('Looks like you entered a wrong email or password.');
});



test('Invalid Login (Invalid Email Format)', async ({ page }) => {
  await page.goto('/');
  const login = new LoginPage(page);
...
test('Invalid Login (Invalid Email Format and Type)', async ({ page }) => {
  await page.goto('/');
  const login = new LoginPage(page);
  ...
  await login.signIn();
  await login.assertToastMessage('Looks like you entered a wrong email or password. Signed up with Google or Apple? Log in using one of the buttons.');


});

test('Invalid Login (Wrong Email Format)', async ({ page }) => {
  await page.goto('/');
  const login = new LoginPage(page);
...
test('Sign In with Apple', async ({ page,context }) => {
...
test('Sign In with Google', async ({ page, context }) => {
  await page.goto('/');
  const login = new LoginPage(page, context); 
